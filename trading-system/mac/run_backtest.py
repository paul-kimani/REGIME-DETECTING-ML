...
def _load_data(
    ...
    from the MT5 bridge if the DB returns insufficient data.
    ...
    try:
        ...
        log.warning("DB unavailable: %s — will try MT5 bridge", exc)

    # ── Attempt 2: MT5 Bridge ─────────────────────────────────────────────
    ...
    if missing:
        try:
            ...
            if not health.get("mt5_connected"):
                raise MT5ConnectionError("MT5 bridge unhealthy")

            pipeline = DataPipeline(mt5, None)
            # Use a generous bar count: days × bars_per_day_per_timeframe
            ...
            log.warning("MT5 bridge unavailable: %s", exc)
...
def _build_components(symbol: str, initial_balance: float) -> dict:
    ...
    Args:
        symbol:          Instrument symbol (used to build per-symbol signal modules).
    ...
    for mod, name in [(momentum, "MomentumModule"), (mr_mod, "MeanReversionModule"),
    ...
    return {
        ...
        "analyzer":     None,  # created after output dir is known
    }
...
def main() -> None:
    ...
    log.info("=== Backtest: %s | %s → %s | folds=%d ===",
             args.symbol, args.start, args.end, args.folds)

    # Load config (best-effort — modules fall back to defaults)
    try:
        get_config()
    except Exception as exc:
    ...
    log.info("Primary data: %d bars (%s → %s)",
             len(primary_df),
    ...
    # ── Exit code: non-zero if consistency check failed ───────────────────
